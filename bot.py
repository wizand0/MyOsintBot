import os
import logging
import asyncio  # Для использования asyncio.to_thread
...
# Обработчик входящих сообщений – логика меню и поиск
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get('search_mode'):
        ...
        await update.message.reply_text("Идет поиск, пожалуйста, подождите...")
        ...
        if results:
            ...
            if len(response) < 3500:
                ...
                # Если результат слишком длинный, то сохраняем его как HTML и отправляем файл
...
#     text = update.message.text.strip().lower()
#
#     # Если пользователь выбирает режим поиска
#     if text == "общий поиск":
#         context.user_data['search_mode'] = 'general'
#         await update.message.reply_text("Введите поисковый запрос для общего поиска:")
#         return
...
#             "1. Выберите режим поиска:\n"
#             "   • 'Общий поиск' – поиск по текстовым столбцам во всех таблицах базы данных.\n"
#             "   • 'Поиск по номеру телефона' – поиск осуществляется только по столбцу phone_number во всех таблицах, где он присутствует.\n"
#             "2. Введите поисковый запрос.\n"
#             "3. Бот выполнит поиск и вернет найденные результаты (ограничено 10 записями из каждой таблицы).\n"
#             "Если возникнут вопросы, свяжитесь с администратором."
#         )
...
#         return
#
#     # Если пользователь уже выбрал режим поиска, то обрабатываем его запрос
#     if context.user_data.get('search_mode'):
#         query_text = update.message.text.strip()
#         mode = context.user_data.get('search_mode')
#         await update.message.reply_text("Идет поиск, пожалуйста, подождите...")
...
#             for row in results:
#                 # Выводим таблицу, из которой получен результат, и пары ключ-значение
#                 row_data = " | ".join(f"{key}: {value}" for key, value in row.items() if key != 'table_name')
#                 response += f"[{row.get('table_name', 'unknown')}] {row_data}\n"
#         else:
#             response = "Ничего не найдено по вашему запросу."
...
def build_html_table(results: list) -> str:
    ...
    Для каждой группы записей:
      ...
      • Затем идут строки с данными. Для каждой записи, содержащей дополнительные поля (кроме ключа table_name),
        значения выводятся в отдельных ячейках. Если у разных записей разное число столбцов, более короткую запись дополняем пустыми ячейками.

    Возвращаемая строка содержит одну HTML‑таблицу, в которой сначала идут все записи из первой таблицы, затем из второй и т.д.
    """
